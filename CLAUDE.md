...
│   ├── admin-bulk.js # 관리자 숨김 대량 번호 추가창
...
- 관리자 관리 화면 안을 650ms 이내 빠르게 세 번 클릭하면 `관리자 모드의 관리자 모드`가 열리며, 현재 선택 모드에 특정 번호를 최대 10,000개까지 한 번에 추가하고 중복 허용을 자동으로 켠다.
...
- 20개를 넘는 대량 추첨 결과는 총인원과 항목별 개수를 큰 글자의 세로 스크롤 목록으로 요약하고, 뽑은 번호 기록은 같은 값을 묶어 표시한다. 핀볼 순위는 30명 초과 시 최근 순위만 표시하되 실제 순위 번호를 유지한다.
...
- 화면 하단의 `.build-info` 수정 버전과 요약 문구는 모든 기능 변경 때 현재 내용에 맞게 함께 갱신한다.
