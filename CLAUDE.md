...
- S자 우회 경로는 서로 겹치지 않는 마른 하강·수중 상승·마른 하강 통로를 곡선 보간해 잇는다. 이동 공에는 기존 세로 본선 경계 보정을 적용하지 않고 끝점까지 여러 프레임에 걸쳐 수렴시킨다.
...
- 기본 네온 보드는 공이 양옆 빈 통로로 곧장 떨어지지 않도록 좌우 벽 핀을 같은 간격과 대칭 교차 각도로 규칙적으로 유지한다.
