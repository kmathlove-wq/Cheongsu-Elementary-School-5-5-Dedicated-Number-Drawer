...
- Guided pinball branches need disjoint entry ranges, constant-width metadata where required, non-crossing centerlines, and shared monotonic progress so the camera follows the actual leader; verify both routes are entered, containment, effects, leader selection, and camera deltas frame by frame.
