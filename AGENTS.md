...
- `basic`, `gumball`, `pinball` 자체 모드에서는 이미 방식이 정해져 있으므로 설정창의 뽑기 방식 선택을 비활성화한다.
- `manitto` 모드는 자체 배정 방식이 있으므로 설정창의 뽑기 방식 선택을 비활성화하고, 설정창에서 마니또 제외 항목을 선택한다.
- 인원수 선택 줄 오른쪽 톱니 버튼으로 설정창을 다시 열 수 있다.
- 설정창에서 효과음 켜기/끄기, 신나는/차분한/아케이드 테마, 기본/공 뽑기/핀볼 미리듣기를 제공한다.
- `song-pinball` 모드는 설정된 뽑기 방식이 기본/공 뽑기/핀볼 중 무엇이든 결과 표시 전 노래 입력창을 띄운다.
