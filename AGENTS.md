...
- 카메라는 한 공을 계속 추적하고 해당 공이 끝나거나 크게 뒤처질 때만 다음 공으로 바꾸며, 한 프레임 이동량을 제한한다. 미니맵 호버 중에는 호버 위치를 따른다.
- 기본 네온 보드는 공이 양옆 빈 통로로 곧장 떨어지지 않도록 좌우 벽 핀의 높이·거리·각도·길이를 엇갈려 자연스럽게 유지한다.
