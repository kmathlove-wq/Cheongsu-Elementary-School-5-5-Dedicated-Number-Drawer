...
│   ├── pinball-maps.js # 핀볼 맵별 장애물 정의
│   ├── pinball-water-renderer.js # 수중 리프트/분기 통로 렌더링
...
| `wheel` | 원판을 돌려 화살표가 가리키는 조각이 선택 인원 수만큼 순차로 뽑히는 돌림판 |
...
- 돌림판 모드는 번호 그리드 대신 원판을 표시하고, 뽑기 버튼을 누르면 원판이 여러 바퀴 빠르게 돌다가 화살표가 가리키는 조각에 멈춰 선택 인원 수만큼 자동으로 이어서 돈다. 조각이 32개 이하면 조각 크기에 맞춰 커지는 숫자를 표시하고(그 이상은 색만), 뽑힌 조각은 별도 표시 없이 바로 원판에서 사라지며 선생님 조각은 금색이다.
...
- 50개를 넘는 대량 추첨 결과는 총인원과 항목별 개수를 큰 글자의 세로 스크롤 목록으로 요약하고, 뽑은 번호 기록은 같은 값을 묶어 표시한다. 핀볼 순위는 30명 초과 시 최근 순위를 표시하고, 휠·드래그·스크롤바로 순위를 탐색하며 아래 화살표로 최신 순위에 바로 이동한다.
...
## 일반 설정창
- 첫 접속 시 기본/공 뽑기/돌림판/핀볼 중 하나의 뽑기 방식을 선택하는 설정창을 띄운다.
- 선택은 현재 번호 모드를 바꾸지 않고 뽑기 방식만 바꾸며, 브라우저에 저장하지 않고 현재 접속 중에만 유지한다.
- `basic`, `manitto`, `gumball`, `wheel`, `pinball` 자체 모드에서는 이미 방식이 정해져 있으므로 설정창의 뽑기 방식 선택 영역을 숨긴다.
- `manitto` 모드는 설정창에서 마니또 제외 항목을 선택한다.
- 인원수 선택 줄 오른쪽 톱니 버튼으로 설정창을 다시 열 수 있다.
- 설정창에서 효과음 켜기/끄기, 신나는/차분한/아케이드 테마, 기본/공 뽑기/돌림판/핀볼 미리듣기를 제공한다.
...
- 핀볼 맵 select 아래 `맵 합치기`/`이 맵 삭제` 버튼으로 2~4개의 기존 맵을 순서대로 골라 이름 붙여 새 맵을 만들거나 지울 수 있으며, `localStorage`(`pinball-merged-maps`)에 저장되어 새로고침해도 유지된다.
...
- 지그재그 협곡은 직선 경계와 날카로운 코너, 곡선 슬라이드는 불규칙한 연속 곡선, 혼돈 공장은 직선/곡선을 혼합한 초장거리 코스를 사용한다.
- 혼돈 공장에는 하강 가속 게이트, 회전하며 상하좌우로 이동하는 긴 핀, 강하게 튕긴 뒤 사라지는 일회용 범퍼가 있다.
- 혼돈 공장의 물 리프트는 아래 수조에 닿은 공을 충돌 없이 위로 올리고, 꼭대기에서 한 번만 최고 속도로 급강하시킨다.
- 혼돈 공장의 수중 상승 코스는 기존 하강 본선 위에 겹치지 않는다. 본선을 중간에서 실제로 끊고, 마른 내리막에서 물로 채운 오르막으로 이어진 뒤 물이 끝나는 위쪽 곡선부터 다시 마른 내리막이 되어 아래 본선에 같은 폭으로 자연스럽게 합류한다.
- 혼돈 공장 중간은 왼쪽의 폭이 줄지 않는 수중 상승 경로와 오른쪽의 구불구불한 일반 하강 경로로 나뉜다. 분기 입출구는 본선 폭을 절반씩 채운 뒤 수직 완충 구간에서 각 통로 폭으로 부드럽게 이어지고, 실제 경로 길이에 따라 일반 길 속도를 계산해 양쪽 예상 통과 시간을 맞춘다.
- 수중 구간은 별도 좌우 다각형 경계 대신 둥근 중심선 스트로크가 통로 전체를 채우도록 그려 안쪽 선이 겹치지 않게 한다. 물길 입출구는 소닉붐을 만들지 않는다.
- 카메라는 본선 Y와 수중 우회로 진행률을 합친 되돌아가지 않는 경기 진행도로 가장 앞선 공 한 개를 고르고, 한 프레임 이동량을 제한해 따라간다. 미니맵 호버 중에는 호버 위치를 따른다.
- 기본 네온 보드는 중앙 핀을 행마다 4개·3개로 교차 배치하고 양옆 벽 핀과 같은 행·각도로 맞춘다. 중앙 끝 핀과 벽 핀이 가까이 중복되지 않으면서 공이 양옆으로 곧장 떨어지지 않게 한다.
- `pegs`, `bumpers`, `spinners`, `balls`, `winners`, `sonicBooms`가 핵심 상태다.
- 결승선(`PLAY_BOT`)을 통과한 순서대로 `winners`에 들어가고 목표 인원 달성 후 `finalize()`가 결과를 반영하며, 미니맵 상수는 `PLAY_BOT` 선언 이후에 두어 TDZ 오류를 피한다.
...
- `mystery`와 `twenty-six` 활성 버튼은 같은 어두운 그라데이션, `gumball` 활성 버튼은 빨강/청록 그라데이션, `wheel` 활성 버튼은 분홍/금색 그라데이션, `pinball` 활성 버튼은 다크 보라 네온, `pinball-teacher` 활성 버튼은 다크 네이비 + 금색 네온.
- `선생님` 셀과 공, 돌림판 조각은 금색 계열로 구분한다.
...
- JavaScript를 포함한 모든 소스 파일은 2,000줄을 넘기지 않고 책임별 모듈로 분리한다.
